...
def load_identity_module():
    # When running in Docker container, module is at /app/services/identity/src/identity.py
    # When running locally, it's at ../../services/identity/src/identity.py
    import sys
    base_docker = "/app/services/identity"
    base_local = os.path.abspath(os.path.join(os.path.dirname(__file__), "..", "..", "services", "identity"))
    
    if os.path.exists(os.path.join(base_docker, "src", "identity.py")):
        base = base_docker
    else:
        base = base_local
...
class FakePool:
    def __init__(self):
        ...
        admin_username = admin_email.split('@', 1)[0] if '@' in admin_email else admin_email
        hashed = hashlib.sha256(admin_password.encode()).hexdigest()
        self.state['users'].append({'id': 1, 'username': admin_username, 'password': hashed, 'domain_id': 1})
...
async def client(identity_module):
    # create admin token using the same secret as the identity service
    secret = identity_module.config.JWT_SECRET
