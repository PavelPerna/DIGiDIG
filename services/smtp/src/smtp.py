    ...
    try:
        if os.path.exists(CONFIG_FILE_PATH):
            with open(CONFIG_FILE_PATH, 'r') as f:
                ...
                logger.info(f"Loaded saved configuration from {CONFIG_FILE_PATH}")
                return saved_config
...
async def save_config_to_file():
    ...
    try:
        ...
        # Save only configurable values (not runtime state)
        config_to_save = {
            key: value for key, value in service_state["config"].items()
...
async def initialize_config():
    """Initialize configuration with persistent storage support"""
    global executor
    
    # Try to load saved configuration
    saved_config = await load_config_from_file()
    if saved_config:
        # Merge saved config with defaults, saved config takes priority
        for key, value in saved_config.items():
            if key in service_state["config"]:
                service_state["config"][key] = value
                logger.info(f"Applied saved config: {key} = {value}")
        
        # Recreate thread pool with saved max_workers setting
        executor.shutdown(wait=True)
        executor = ThreadPoolExecutor(max_workers=service_state["config"]["max_workers"])
        logger.info(f"Thread pool initialized with {service_state['config']['max_workers']} workers")
    else:
...
async def update_config(config: Dict[str, Any]):
    """Update SMTP service configuration with persistent storage"""
    try:
        # Update configuration in memory
        for key, value in config.items():
            if key in service_state["config"]:
                old_value = service_state["config"][key]
                service_state["config"][key] = value
        ...
        if "max_workers" in config:
            executor.shutdown(wait=True)
            executor = ThreadPoolExecutor(max_workers=service_state["config"]["max_workers"])
            logger.info(f"Thread pool restarted with {service_state['config']['max_workers']} workers")
        
        logger.info(f"Configuration updated and saved persistently")
        return {
            "status": "success",
